...
@section('title', __('design_options.deleted_design_options'))
@section('page-title', __('design_options.deleted_design_options'))
...
@section('content')
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">{{ __('design_options.deleted_design_options') }}</h2>
            <p class="text-sm text-gray-600 mt-1">{{ __('design_options.manage_deleted_design_options') }}</p>
        </div>
        <div class="mt-4 md:mt-0">
            <a href="{{ route('design_options.index') }}"
                ...
                {{ __('design_options.back_to_design_options') }}
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        <form method="GET" action="{{ route('design_options.trashed') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <!-- Name -->
            ...
            <table class="w-full">
                <thead class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center text-xs font-semibold uppercase tracking-wider">
                            {{ __('design_options.deleted_at') }}</th>
                        <th class="px-6 py-4 text-center text-xs font-semibold uppercase tracking-wider">
                    ...
                    @forelse($designOptions as $designOption)
                        <tr class="hover:bg-purple-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center text-sm text-gray-600">
                                <div>{{ $designOption->deleted_at?->format('d M, Y') }}</div>
                                <div class="text-xs text-gray-500">{{ $designOption->deleted_at?->diffForHumans() }}</div>
                            </td>
                            <td class="px-6 py-4">
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center text-gray-500">
                                    ...
                                    <p class="text-lg font-semibold">{{ __('design_options.no_deleted_design_options') }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
