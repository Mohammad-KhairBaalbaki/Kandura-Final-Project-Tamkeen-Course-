...
@section('title', __('design_options.edit_design_option'))
@section('page-title', __('design_options.edit_design_option'))
...
@section('content')
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">{{ __('design_options.edit_design_option') }}</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type"
                        ...
                        <option value="dome" {{ old('type', $designOption->type) === 'dome' ? 'selected' : '' }}>
                            {{ __('design_options.type_dome') }}
                        </option>
                        <option value="fabric" {{ old('type', $designOption->type) === 'fabric' ? 'selected' : '' }}>
                            {{ __('design_options.type_fabric') }}
                        </option>
                        <option value="sleeve" {{ old('type', $designOption->type) === 'sleeve' ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="is_active"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition">
                        <option value="1" {{ old('is_active', $designOption->is_active ? '1' : '0') === '1' ? 'selected' : '' }}>
                            {{ __('design_options.active') }}
                        </option>
                        <option value="0" {{ old('is_active', $designOption->is_active ? '1' : '0') === '0' ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="name[en]" value="{{ old('name.en', $designOption->getTranslation('name', 'en')) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition">
                    @error('name.en')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="name[ar]" value="{{ old('name.ar', $designOption->getTranslation('name', 'ar')) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition">
                    @error('name.ar')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
