...
<body class="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 flex items-center justify-center p-6"
    dir="{{ $isRtl ? 'rtl' : 'ltr' }}">
    <div class="w-full max-w-lg bg-white shadow-xl rounded-2xl p-8 border border-gray-100 text-center">
        ...
        <h1 class="text-2xl font-bold text-gray-900 mb-2">{{ __('Payment Successful') }}</h1>
        <p class="text-sm text-gray-600 mb-6">
            {{ __('Your payment was completed successfully.') }}
        </p>
        ...
        <div class="mt-6 space-y-3">
            <a href="{{ route('dashboard') }}"
                class="inline-block w-full px-4 py-2 rounded-lg bg-green-600 text-white font-semibold hover:bg-green-700 transition">
                {{ __('Success') }}
            </a>
        </div>
    </div>
